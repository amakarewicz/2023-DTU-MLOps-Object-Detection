...
def main(config: DictConfig):
    ...
    #     raise Exception(
    #         'The "model_output_dir" path ({})
    # could not be found'.format(output_dir)
    #     )

    loader = LoadImages(root_dir=HydraConfig.get().runtime.cwd)
    dataloader = loader.get_dataloader(
        config.predict.dataset, config.predict.batch_size, shuffle=False
    )
    batch = next(iter(dataloader))

    #     # %% Load local config in output directory
    #     output_config_path = os.path.join(output_dir,
    #     ".hydra", "config.yaml")
    ...
    # output_checkpoint_latest_path, config=output_config)
    ...
    output_prediction_file = os.path.join(
        output_prediction_dir, "results.json"
    )
